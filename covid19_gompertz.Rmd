...
---
# * Version 1.0
Uses data from package covid19  (see https://covid19datahub.io) to fit single and double Gompertz equations to model one- and two-wave  COVOD19 spread, respectively, by means of a Bayesian MCMC approach.
...
* `full_data`             : Loaded with one week (7 days) lag. 
...
A list of the countries included in the working database `dat` can be obtained using `include_countries(sf_dat, "Pop+Dens")` and for those excluded `exclude_countries(sf_dat, "Pop+Dens")`
...
```{r}
top_10_ranking(df = latest_figures, type = "Fatalities")
top_10_ranking(df = latest_figures, type = "Daily_Fatalities")
top_10_ranking(df = latest_figures, type = "Density_Fatalities")
```
...
# MCMC simulation
MCMC estimation of a Gompertz function for each of the `type` in `by_cases` and countries in `by_country`
```{r}
# Needed to execute the MCMC and visualization
fit_range    <- 0:projected_days

# List of countries of interest
by_country <- c("Canada", "Germany")
...
for (by_type in by_cases){
  ...
  country_list_output_mcmc <- foreach (i = 1:length(by_country)) %dopar% {
    ...
    run_mcmc_simulation_2( index_country         = i                    , 
                           ...
                           number_steps_run_init = 2e4                  , 
                           number_steps_run_max  = 1e7 
                           )
  }
...
Active cases can by estimated as a by-product of the previous results.
```{r}
...
Visualizing the results by calling directly each element of the list `case_list_output_vis`:
```{r}
case_list_output_vis[["Active_Cases"]][[by_country[1]]][["Case"]]
case_list_output_vis[["Active_Cases"]][[by_country[1]]][["Daily_Case"]]
```
